...
import { disconnectYouTube } from '@/utils/youtube'
...
export default function YouTubeConnectModal({ connected }: YouTubeConnectModalProps) {
  ...
  const handleDisconnect = async () => {
    if (!profile) return
    setIsLoading(true)
    const result = await disconnectYouTube(profile.uuid)
    setIsLoading(false)
    if (result.success) {
      setOpen(false)
      router.push('/dashboard/settings?toast=YouTube account disconnected')
    }
  }
        ...
        {connected ? (
          <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Are you sure you wish to disconnect your YouTube account?
              </p>
